...
namespace alpaka
{
    ...
    namespace detail
    {
        ...
        struct TaskFillCpuBase
        {
                ...
                , m_dstMemNative(reinterpret_cast<std::uint8_t*>(getPtrNative(view)))
            {
                ...
                if constexpr(TDim::value > 1)
                    ALPAKA_ASSERT(m_extentWidth * static_cast<ExtentSize>(sizeof(Elem)) <= m_dstPitchBytes[TDim::value - 2]);
        ...
        struct TaskFillCpu : public TaskFillCpuBase<TDim, TView, TExtent, TValue>
        {
            ...
            ALPAKA_FN_HOST auto operator()() const -> void
            {
                ...
                if (static_cast<std::size_t>(this->m_extent.prod()) != 0u)
                {
                    ...
                    {
                        std::uint8_t* rowBase =
                            this->m_dstMemNative + (castVec<DstSize>(idx) * pitchBytesWithoutOutmost).sum();
                        Elem* row = reinterpret_cast<Elem*>(rowBase);

                        for (std::size_t i = 0; i < static_cast<std::size_t>(this->m_extentWidth); ++i)
                        {
                            row[i] = this->m_value;
                        }
                    });
        ...
        struct TaskFillCpu<DimInt<1u>, TView, TExtent, TValue>
            : public TaskFillCpuBase<DimInt<1u>, TView, TExtent, TValue>
        {
            using Base = TaskFillCpuBase<DimInt<1u>, TView, TExtent, TValue>;
            using Base::Base;
            using Elem = typename Base::Elem;

            ALPAKA_FN_HOST auto operator()() const -> void
            {
                Elem* ptr = reinterpret_cast<Elem*>(this->m_dstMemNative);
                for (std::size_t i = 0; i < static_cast<std::size_t>(this->m_extentWidth); ++i)
                {
                    ptr[i] = this->m_value;
                }
    ...
    struct CreateTaskFill<TDim, DevCpu>
    {
            ...
            TExtent const& extent)
        {
            ...
            return alpaka::detail::TaskFillCpu<TDim, TView, TExtent, TValue>{
                std::forward<TViewFwd>(view), value, extent};
        }
    };
    }
